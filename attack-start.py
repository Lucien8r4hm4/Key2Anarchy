...
for t in mythreads:
    t.join()
admin@ip-10-0-1-161:/opt/cerberus$ cat attack-start.py
import socket
from threading import Thread
import os
...
class myThread(Thread):
    ...
    def run(self):
        while True :
            data = con.recv(2048)
            print("We received this :", data)
            if data[0:4] == b'LETS':
                file = open("/opt/cerberus/ATTACK_START","r")
                attack = file.read(1)
                file.close()
                if attack == "1":
                    con.send(b"GO\x0d\x0a\x00")
                else:
                    con.send(b"WAIT\x0d\x0a\x00")
